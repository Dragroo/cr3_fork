...
class ServiceClientsDemo(Node):
    ...
    def accj_client(self,r):

        accj_handle = self.create_client(AccJ, '/dobot_bringup/srv/AccJ')

        accj_req = AccJ.Request()
        accj_req.r = r

        while not accj_handle.wait_for_service(timeout_sec=1.0):
            self.get_logger().info("service /dobot_bringup/srv/AccJ not available, waiting again...")    
        accj_future = accj_handle.call_async(accj_req)
        rclpy.spin_until_future_complete(self,accj_future)

        accj_res = accj_future.result()
