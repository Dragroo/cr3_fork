...
class TFListener(Node):
    ...
    def __init__(self, name):
        ...
        self.pub=self.create_publisher(PoseStamped,"my_trajectory",10)
        self.declare_parameter('source_frame', 'Link6')             # 创建一个源坐标系名的参数
        self.source_frame = self.get_parameter(                     # 优先使用外部设置的参数值，否则用默认值
            'source_frame').get_parameter_value().string_value
        self.declare_parameter('target_frame', 'base_link')             # 创建一个目标坐标系名的参数
    ...
    def on_timer(self):
        ...
        self.pose_stamped.pose.position.x=pos.x
        self.pose_stamped.pose.position.y=pos.y
        self.pose_stamped.pose.position.z=pos.z
        self.pose_stamped.pose.orientation.w=quat.w
        self.pose_stamped.pose.orientation.x=quat.x
        self.pose_stamped.pose.orientation.y=quat.y
        self.pose_stamped.pose.orientation.z=quat.z
        ...
        # self.get_logger().info('x: %f, y:%f, z:%f ' % (pos.x, pos.y, pos.z))
